...
# on extrait du contenu : contenu du chapitre, titre du chapitre, et métadonnées.
my $n = 0;
my $continuer = 1;
...
# A partir de la première traversée de l'HTML, nous récupérons l'information du nombre de chapitres (nommé $n).
...
while ($n < $nbdechapitres){
   ...
   $page = get('https://www.fanfiction.net/s/'."$_[0]".'/'.($n+1).'/'."$_[1]");  # Nous reprenons ici les segments de l'URL
                                                                                 # qui nous intéressent, obtenus depuis l'URL
      ...
      for(my $i=0; $i<scalar(@lignes); $i++) {
            ...
            if ($lignes[$i] =~ /<div class='storytext xcontrast_txt nocopy' id='storytext'>/) {
               $indiceInitial = $i;   # Pour la capture du chapitre, et éviter les nombreuses différences
                                      ...
                                      # s'enclenche ici et permettera de concaténer toutes les phrases récoltées.
            }
            if ($lignes[$i] =~ /<\/p>/) {
            ...
            # On récupère la description/résumé si nous ne l'avons pas encore récupéré(e).
            if ($resume eq '' and $lignes[$i] =~ /<div style='margin-top:2px' class='xcontrast_txt'>(.+)<\/div>/){
               $resume = $1;
            }
            
            # On récupère le titre du livre si nous ne l'avons pas encore récupéré.
            if ($titrelivre eq '' and $lignes[$i] =~ /<b class='xcontrast_txt'>(.+)<\/b>/){
               $titrelivre = $1;
            }
            
            # On récupère une série de métadonnées.
            ...
            # On récupère le jour et le mois de publication si nous ne les avons pas encore récupérés.
...
# Ici nous téléchargeons la couverture du livre et la renommons cover.jpeg ;
# ce qui aura pour effet d'écraser les covers précédentes

getstore("$cover","cover.jpeg");

# Avec toutes les données récupérées, nous pourrons lancer la création de l'epub lui-même
...
# En fonction des informations récupérées via l'HTML nous connaissons
# la date complète ou incomplète de publication.
...
#print Dumper $epub;  # Pour voir la progression de la création de chaque Epub, Dumper $epub est utile
                      # Pour garder l'interface de l'utilisateur le plus épurée possible, nous désactivons ceci.
...
$epub->add_identifier('Fanfiction au format epub grâce à Epub_Creator.pl');   # Information sur le programme
...
# On crée chacun des chapitres et on les ajoute dans la table des matières.
...
sub get_all_epub {
        ...
        for(my $i=0; $i<scalar(@lignes); $i++) {
             ...
             if ($lignes[$i] =~ /<div class='storytext xcontrast_txt nocopy' id='storytext'>/) {
                $indiceInitial = $i; # Pour la capture du chapitre, et éviter les nombreuses différences
                                     # entre les pages HTML (chaque auteur possède sa propre convention)
                                     # nous initions un compteur qui (dans la boucle for qui suit)
                                     # s'enclenche ici et permettra de concaténer toutes les phrases récoltées.
   ...
   # Vérification que le nombre de chapitres existe,
   # sinon nous indiquons qu'il n'y a qu'un chapitre
   if (!defined $nbdechapitres) {
       $nbdechapitres = 1 ;
   }

...
# 2 - En opérant une navigation des critères de sélection des fanfictions disponibles
#======================================================================
...
#                   la navigation entre les différents critères de sélection et
#                   la proposition de fanfictions correspondantes.
...
# l'option 1 renvoie au module_epub.pm et plus spécifiquement à la fonction getepub.
# l'option 2 renvoie à navigation.pm qui lui-même renvoie au module_epub.pm et la fonction get_all_epub.
...
while($loop <2){
    ...
    while ($loop <1){
       ...
       option 2) Naviguer dans la base de données du site www.fanfiction.net et sélectionner d'ici quelle(s) fanfiction(s) vous souhaitez télécharger au format epub. \n
       ";
       ...
       if ($choix1 == 1){
            ...
            $URL =~ /\/s\/(.+?)\/[0-9]+\/(.+)/;   # Nous en profitons pour récupérer depuis l'URL fournie les deux parties
            my $premierepartie = $1;              # qui seront nécessaires à getepub et nous la donnons en argument de la fonction.
            my $deuxiemepartie = $2;
        ...
        # le programme exclut les fanfictions qui ne respectent pas les critères de sélection.
  ...
  print "\n\nEncore envie de créer des epubs ? \nTapez oui : pour convertir de nouvelles fanfictions en epub.\n";
  mise_en_page::espace;
